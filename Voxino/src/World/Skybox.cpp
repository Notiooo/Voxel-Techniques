...
GLenum Skybox::deduceImageFormat() const
{
    switch (mNrChannels)
    {
        ...
        default: spdlog::critical("Unknown image format: nrChannels:{}", mNrChannels); return 0;
    }
}

void Skybox::loadTextureFace(const std::vector<std::string>& faces, unsigned faceId)
{
    unsigned char* mData = stbi_load(faces[faceId].c_str(), &mWidth, &mHeight, &mNrChannels, 0);
    const auto format = deduceImageFormat();
    if (mData)
    {
