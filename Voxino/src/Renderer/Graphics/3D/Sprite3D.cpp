...
{
    const auto max = static_cast<float>(std::max(mTexture.width(), mTexture.height()));
...
{
    const auto newDimensions = glm::vec2{width, width / mTexture.aspectRatio()};
    const auto max = static_cast<float>(std::max(newDimensions.x, newDimensions.y));
...
{
    const auto newDimensions = glm::vec2{height * mTexture.aspectRatio(), height};
    const auto max = static_cast<float>(std::max(newDimensions.x, newDimensions.y));
    mDimensionsNormalized = {normalize(newDimensions.x, 0, max),
                             normalize(newDimensions.y, 0, max)};
    updateModel();
