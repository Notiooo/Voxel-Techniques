...
uniform usamplerBuffer u_BrickmapOccupancy;
...
vec4 castRay(vec3 rayOrigin, vec3 rayDirection, out float depth)
{
    ...
    if (u_DrawRayIterations)
    {
        ...
        float normalizedIterations = float(numIterations) / float(u_MaxRayIterationsVisualization);
        return vec4(vec3(normalizedIterations), 1.0);
    }
    if (!hasIntersection)
    {
        discard;
