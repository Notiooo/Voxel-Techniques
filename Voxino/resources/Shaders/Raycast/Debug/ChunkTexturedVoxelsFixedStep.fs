...
uniform float u_FixedStepSize = 0.1;
uniform bool u_SwitchAlgorithm = false;
uniform bool u_DrawNormals = false;
uniform bool u_DrawDepth = false;
...
vec3 mapToTexCoordFixedStep(vec3 worldCoord)
{
    return worldCoord / vec3(u_WorldSize);
}

bool sampleWorldFixedStep(vec3 worldCoord, out vec4 worldData)
{
    vec3 texCoord = mapToTexCoordFixedStep(worldCoord);
    ivec3 voxelCoord = ivec3(texCoord * vec3(textureSize(u_VoxelWorld, 0)));
    worldData = texelFetch(u_VoxelWorld, voxelCoord, 0);
    return true;
}
...
vec3 calculateNormalForFixedStep(vec3 currentPos)
{
    ...
    {
        hitNormal = vec3(sign(offset.x), 0.0, 0.0);
    }
    else if (abs(offset.y) > abs(offset.x) && abs(offset.y) > abs(offset.z))
    {
        hitNormal = vec3(0.0, sign(offset.y), 0.0);
    }
    else
    {
        hitNormal = vec3(0.0, 0.0, sign(offset.z));
    }

    return hitNormal;
}

bool traverseWorldWithFixedStep(vec3 rayOrigin, vec3 rayDirection, float tNear, float tFar, out vec4 color, out vec3 normal, out float distance, out int numIterations)
{
    tNear = max(0, tNear) + 0.001;
    vec3 currentPos = rayOrigin + rayDirection * tNear;
    distance = tNear;

    while (distance <= tFar && isInWorldBounds(currentPos))
    {
        ++numIterations;
        atomicCounterIncrement(rayIterationCounter);

        vec4 voxelData;
        if (sampleWorldFixedStep(currentPos, voxelData))
        {
            if (voxelData.w != 0)
            {
                color = voxelData;
                normal = calculateNormalForFixedStep(currentPos);
                return true;
            }
        }

        currentPos += rayDirection * u_FixedStepSize;
        distance += u_FixedStepSize;
    }
    return false;
}
...
bool intersectWorld(vec3 rayOrigin, vec3 rayDirection, out vec4 voxelData, out float distance, out vec3 normal, out int numIterations)
{
    ...
    if(u_SwitchAlgorithm)
    {
        return traverseWorld(rayOrigin, rayDirection, tNear, tFar, voxelData, outerSideNormalAxis, normal, distance, numIterations);
    }
    else
    {
        return traverseWorldWithFixedStep(rayOrigin, rayDirection, tNear, tFar, voxelData, normal, distance, numIterations);
    }
...
vec4 castRay(vec3 rayOrigin, vec3 rayDirection, out float depth)
{
    ...
    if(u_DrawDepth)
    {
        return vec4(vec3(depth * 100.f), 1.0);
    }
